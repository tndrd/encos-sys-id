...
void SignalGenerator::setMode(int mode) {
    m_mode = mode;
}

void SignalGenerator::playSignalLoop(const std::vector<float> &signal, float dtms) {
    timespec ts_next;
    ...
    for (size_t i = 0; i < signal.size(); ++i) {
        ...
        float pos = (m_mode == 0) ? signal[i] : 0;
        float kp  = (m_mode == 0) ? 500 : 0;

        float vel = (m_mode == 1) ? signal[i] : 0;
        float kd  = (m_mode == 1) ? 5: 0;

        float trq = (m_mode == 2) ? signal[i] : 0;
...
SignalGenerator::playSignal(const std::vector<float> &signal, float dtms) {
    std::thread thread(SignalGenerator::playSignalRoutine, this, std::cref(signal), dtms);
    thread.join();
...
void SignalGenerator::brake(float kp, float kd) {
    ...
    m_do_brake = true;
    m_brake_thread = std::make_unique<std::thread>(SignalGenerator::brakeRoutine, this, kp, kd);
}

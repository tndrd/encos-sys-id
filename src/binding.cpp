...
PYBIND11_MODULE(PYTHON_MODULE_NAME, m) {
    m.doc() = "ENCOS drive identification tools.\n"
    ...
    py::class_<SignalGenerator>(m, "SignalGenerator")
        .def(py::init<const std::string &, uint8_t>(),
             py::arg("interface") = "can0",
             py::arg("drive_id") = 1)

        .def("play_signal", &SignalGenerator::playSignal, py::arg("signal"), py::arg("dtms"))
        .def("brake", &SignalGenerator::brake)
        .def("release", &SignalGenerator::release)
        .def("set_mode", &SignalGenerator::setMode);
}